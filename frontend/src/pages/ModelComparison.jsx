...
export default function ModelComparison() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            return (
              <div
                key={m.key}
                className={`card-raised p-5 border relative overflow-hidden transition-all duration-200 ${
                  isWinner
                    ? 'border-accent-500 bg-gradient-to-br from-accent-500/15 via-surface-900 to-surface-900 shadow-md ring-1 ring-accent-500/30'
                    : 'border-surface-700/60 bg-surface-900/80'
                }`}
              >
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-bold text-surface-50 flex items-center gap-2">
                    <Cpu size={16} className={isWinner ? 'text-accent-500 dark:text-accent-400' : 'text-surface-400'} />
                    {m.title}
                  </h3>
                  <span className="text-[10px] px-2 py-0.5 rounded-full font-bold bg-surface-800 border border-surface-700 text-surface-300 font-mono">
                    {m.badge}
                  </span>
                </div>

                <div className="grid grid-cols-3 gap-2 text-center my-3 py-2.5 bg-surface-800/80 rounded-xl border border-surface-700/50">
                  <div>
                    <p className="text-[10px] text-surface-400 uppercase tracking-wider font-semibold">Mean ROC-AUC</p>
                    <p className="font-data text-base font-extrabold text-surface-50 mt-0.5">
                      {data.mean_auc ? data.mean_auc.toFixed(3) : '—'}
                    </p>
                  </div>
                  <div>
                    <p className="text-[10px] text-surface-400 uppercase tracking-wider font-semibold">Mean F1</p>
                    <p className="font-data text-base font-extrabold text-surface-50 mt-0.5">
                      {data.mean_f1 ? data.mean_f1.toFixed(3) : '—'}
                    </p>
                  </div>
                  <div>
                    <p className="text-[10px] text-surface-400 uppercase tracking-wider font-semibold">Accuracy</p>
                    <p className="font-data text-base font-extrabold text-surface-50 mt-0.5">
                ...
                <p className="text-xs text-surface-400 text-right mt-1 font-mono">
                  Evaluated across <span className="font-bold text-surface-200">{data.experiment_count || 30} seeds</span>
                </p>
              </div>
            )
          })}
      ...
      <div className="card-raised p-5 bg-surface-900 border border-surface-700/60">
        <h2 className="text-sm font-bold uppercase tracking-wider text-surface-50 mb-3 flex items-center gap-2">
          <Sparkles size={16} className="text-accent-500 dark:text-accent-400" />
          Train On-Demand Experiment
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
          <div>
            <label className="block text-xs font-semibold text-surface-400 mb-1">Select Architecture</label>
            <select
              value={customModel}
          ...
          <div>
            <label className="block text-xs font-semibold text-surface-400 mb-1">Random Split Seed</label>
            <input
              type="number"
              value={customSeed}
          ...
          <div>
            <label className="block text-xs font-semibold text-surface-400 mb-1">Split Ratio</label>
            <input
              type="text"
              value="75% Train / 25% Test (Subject Split)"
              disabled
              className="input text-xs text-surface-400 opacity-80"
            />
          </div>
          ...
          <div>
            <button
              ...
              disabled={training}
              className="w-full btn-teal py-2 text-xs flex items-center justify-center gap-1.5 font-semibold"
            >
        ...
        {trainResult && (
          <div className="mt-4 p-4 rounded-xl bg-surface-800/80 border border-accent-500/30 grid grid-cols-2 md:grid-cols-4 gap-3 text-center">
            <div>
              <p className="text-[10px] text-surface-400 uppercase font-semibold">Test Accuracy</p>
              <p className="font-data text-base font-extrabold text-success-500 dark:text-success-400 mt-0.5">
                {(trainResult.metrics.accuracy * 100).toFixed(1)}%
              </p>
            </div>
            <div>
              <p className="text-[10px] text-surface-400 uppercase font-semibold">Test ROC-AUC</p>
              <p className="font-data text-base font-extrabold text-accent-500 dark:text-accent-400 mt-0.5">
                {trainResult.metrics.auc.toFixed(4)}
              </p>
            </div>
            <div>
              <p className="text-[10px] text-surface-400 uppercase font-semibold">F1 Score</p>
              <p className="font-data text-base font-extrabold text-surface-50 mt-0.5">
                {trainResult.metrics.f1_score.toFixed(4)}
              </p>
            </div>
            <div>
              <p className="text-[10px] text-surface-400 uppercase font-semibold">Features Evaluated</p>
              <p className="font-data text-base font-extrabold text-surface-200 mt-0.5">
      ...
      {/* ── 30 Experiments Historical Log Table (with ResponsiveTable) ── */}
      <div className="card-raised p-5 bg-surface-900 border border-surface-700/60">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="input pl-8 py-1 text-xs w-32"
              />
            </div>
          </div>
        </div>
        ...
        <ResponsiveTable minWidth="600px">
          <thead>
            <tr className="border-b border-surface-700/60 bg-surface-800/60 text-surface-400 uppercase text-[11px] font-bold">
              <th className="py-2.5 px-3 pl-4">Exp #</th>
              ...
              <th className="py-2.5 px-3 pr-4 font-mono">Accuracy</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-surface-700/40">
            {filteredExperiments.slice(0, 40).map((exp, i) => (
              <tr key={i} className="hover:bg-surface-800/40 transition-colors">
                <td className="py-2 px-3 pl-4 font-mono font-bold text-surface-300">{exp.experiment_number}</td>
                ...
                <td className="py-2 px-3 font-data text-surface-400">{exp.seed}</td>
                <td className="py-2 px-3 font-data font-bold text-surface-50">{exp.auc.toFixed(4)}</td>
                <td className="py-2 px-3 font-data text-surface-200">{exp.f1_score.toFixed(4)}</td>
                <td className="py-2 px-3 pr-4 font-data text-surface-300">{(exp.accuracy * 100).toFixed(1)}%</td>
              </tr>
            ))}
          </tbody>
        </ResponsiveTable>
